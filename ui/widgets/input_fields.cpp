...
const auto kObjectReplacement = QString::fromRawData(&kObjectReplacementCh, 1);
...
const auto kNewlineChars = QString("\r\n") + QChar(0xfdd0)  // QTextBeginningOfFrame
                           ...
                           + QChar(QChar::ParagraphSeparator) + QChar(QChar::LineSeparator);
...
InputDocument::InputDocument(QObject *parent, const style::InputField &st) : QTextDocument(parent), _st(st) {
  Emoji::Updated() | rpl::start_with_next([=] { _emojiCache.clear(); }, _lifetime);
}

QVariant InputDocument::loadResource(int type, const QUrl &name) {
  if (type != QTextDocument::ImageResource || name.scheme() != qstr("emoji")) {
  ...
  auto result = [&] {
    if (const auto emoji = Emoji::FromUrl(name.toDisplayString())) {
      const auto height = std::max(_st.font->height * style::DevicePixelRatio(), Emoji::GetSizeNormal());
...
QString GetFullSimpleTextTag(const TextWithTags &textWithTags) {
  ...
  return ((tag.offset <= from) && (tag.offset + tag.length >= till)) ? (tag.id == kTagPre ? kTagCode : tag.id)
                                                                     : QString();
}
 ...
 public:
  ...
  void feed(const QString &randomTagId, int currentPosition) {
    ...
    if (!_currentTagId.isEmpty()) {
      const auto tag = TextWithTags::Tag{_currentStart, currentPosition - _currentStart, _currentTagId};
      if (tag.length > 0) {
...
class TagSearchItem {
 public:
  ...
  void fill(const QString &text, Edge edge, const TagStartExpression &expression) {
    const auto length = text.size();
    const auto &tag = expression.tag;
    const auto tagLength = tag.size();
    const auto isGoodBefore = [&](QChar ch) {
      return expression.goodBefore.isEmpty() || (expression.goodBefore.indexOf(ch) >= 0);
    };
    const auto isBadAfter = [&](QChar ch) {
      return !expression.badAfter.isEmpty() && (expression.badAfter.indexOf(ch) >= 0);
    };
    const auto isBadBefore = [&](QChar ch) {
      return !expression.badBefore.isEmpty() && (expression.badBefore.indexOf(ch) >= 0);
    };
    const auto isGoodAfter = [&](QChar ch) {
      return expression.goodAfter.isEmpty() || (expression.goodAfter.indexOf(ch) >= 0);
    };
    const auto check = [&](Edge edge) {
      if (_position > 0) {
        const auto before = text[_position - 1];
        if ((edge == Edge::Open && !isGoodBefore(before)) || (edge == Edge::Close && isBadBefore(before))) {
          return false;
        }
      }
      if (_position + tagLength < length) {
        const auto after = text[_position + tagLength];
        if ((edge == Edge::Open && isBadAfter(after)) || (edge == Edge::Close && !isGoodAfter(after))) {
          return false;
        }
      }
      return true;
...
const std::vector<TagStartExpression> &TagStartExpressions() {
  static auto cached = std::vector<TagStartExpression>{
      {kTagBold, TextUtilities::MarkdownBoldGoodBefore(), TextUtilities::MarkdownBoldBadAfter(),
       TextUtilities::MarkdownBoldBadAfter(), TextUtilities::MarkdownBoldGoodBefore()},
      {kTagItalic, TextUtilities::MarkdownItalicGoodBefore(), TextUtilities::MarkdownItalicBadAfter(),
       TextUtilities::MarkdownItalicBadAfter(), TextUtilities::MarkdownItalicGoodBefore()},
      //{
      //	kTagUnderline,
      ...
      {kTagCode, TextUtilities::MarkdownCodeGoodBefore(), TextUtilities::MarkdownCodeBadAfter(),
       TextUtilities::MarkdownCodeBadAfter(), TextUtilities::MarkdownCodeGoodBefore()},
      {kTagPre, TextUtilities::MarkdownPreGoodBefore(), TextUtilities::MarkdownPreBadAfter(),
...
class MarkdownTagAccumulator {
 public:
  ...
  MarkdownTagAccumulator(std::vector<InputField::MarkdownTag> *tags)
      : _tags(tags), _expressions(TagStartExpressions()), _tagIndices(TagIndices()), _items(_expressions.size()) {
  }
  ...
  void feed(const QString &text, int adjustedTextLength, const QString &textTag) {
    if (!_tags) {
      return;
    }
    ...
    while (true) {
      for (; tryFinishTag != _currentFreeTag; ++tryFinishTag) {
        ...
        const auto atLeastOffset = tag.internalStart + tag.tag.size() + 1 - _currentInternalLength;
        _items[tagIndex].applyOffset(atLeastOffset);

        fillItem(tagIndex, text, Edge::Close);
 ...
 private:
  void finishTag(int index, int offsetFromAccumulated, bool closed) {
    ...
    if (tag.internalLength < 0) {
      tag.internalLength = _currentInternalLength + offsetFromAccumulated - tag.internalStart;
      tag.adjustedLength = _currentAdjustedLength + offsetFromAccumulated - tag.adjustedStart;
      tag.closed = closed;
    }
  ...
  bool finishByNewline(int index, const QString &text, int tagIndex) {
    Expects(_tags != nullptr);
    Expects(index >= 0 && index < _tags->size());
    ...
    const auto endPosition = newlinePosition(text, std::max(0, tag.internalStart + 1 - _currentInternalLength));
  ...
  void startTag(int offsetFromAccumulated, const QString &tag) {
    ...
    const auto newTag = InputField::MarkdownTag{_currentInternalLength + offsetFromAccumulated,
                                                -1,
...
void RemoveDocumentTags(const style::InputField &st, not_null<QTextDocument *> document, int from, int end) {
...
QTextCharFormat PrepareTagFormat(const style::InputField &st, QString tag) {
...
int ProcessInsertedTags(const style::InputField &st, not_null<QTextDocument *> document, int changedPosition,
                        int changedEnd, const TextWithTags::Tags &tags, InputField::TagMimeProcessor *processor) {
  ...
  for (const auto &tag : tags) {
    ...
    if (tagTo > tagFrom && !tagId.isEmpty()) {
      ...
      if (applyNoTagFrom < tagFrom) {
        RemoveDocumentTags(st, document, applyNoTagFrom, tagFrom);
      }
...
const InstantReplaces &InstantReplaces::Default() {
  static const auto result = [] {
    ...
    result.add(":shrug:", QChar(175) + QString("\\_(") + QChar(12484) + ")_/" + QChar(175));
...
FlatInput::FlatInput(QWidget *parent, const style::FlatInput &st, rpl::producer<QString> placeholder, const QString &v)
  ...
  _placeholderFull.value() | rpl::start_with_next([=](const QString &text) { refreshPlaceholder(text); }, lifetime());

  style::PaletteChanged() | rpl::start_with_next([=] { updatePalette(); }, lifetime());
  updatePalette();
  ...
  connect(this, &FlatInput::selectionChanged, [] { Integration::Instance().textActionsUpdated(); });
...
void FlatInput::paintEvent(QPaintEvent *e) {
  ...
  p.setBrush(anim::brush(_st.bgColor, _st.bgActive, placeholderFocused));
  {
    ...
    p.drawRoundedRect(QRectF(0, 0, width(), height())
                          .marginsRemoved(QMarginsF(_st.borderWidth / 2., _st.borderWidth / 2., _st.borderWidth / 2.,
                                                    _st.borderWidth / 2.)),
  ...
  const auto placeholderOpacity = _placeholderVisibleAnimation.value(_placeholderVisible ? 1. : 0.);
...
void FlatInput::focusInEvent(QFocusEvent *e) {
  if (!_focused) {
    _focused = true;
    _placeholderFocusedAnimation.start([=] { update(); }, 0., 1., _st.phDuration);
    update();
  }
...
void FlatInput::focusOutEvent(QFocusEvent *e) {
  if (_focused) {
    _focused = false;
    _placeholderFocusedAnimation.start([=] { update(); }, 1., 0., _st.phDuration);
    update();
  }
...
void FlatInput::updatePlaceholder() {
  ...
  if (_placeholderVisible != placeholderVisible) {
    ...
    _placeholderVisibleAnimation.start([=] { update(); }, _placeholderVisible ? 0. : 1., _placeholderVisible ? 1. : 0.,
                                       _st.phDuration);
  }
}
...
void FlatInput::keyPressEvent(QKeyEvent *e) {
  ...
  if (_customUpDown && (e->key() == Qt::Key_Up || e->key() == Qt::Key_Down || e->key() == Qt::Key_PageUp ||
...
InputField::InputField(QWidget *parent, const style::InputField &st, rpl::producer<QString> placeholder,
                       const QString &value)
    : InputField(parent, st, Mode::SingleLine, std::move(placeholder), {value, {}}) {
}

InputField::InputField(QWidget *parent, const style::InputField &st, Mode mode, rpl::producer<QString> placeholder,
                       const QString &value)
    : InputField(parent, st, mode, std::move(placeholder), {value, {}}) {
}
...
InputField::InputField(QWidget *parent, const style::InputField &st, Mode mode, rpl::producer<QString> placeholder,
  ...
  connect(_inner.get(), &Inner::selectionChanged, [] { Integration::Instance().textActionsUpdated(); });

  const auto bar = _inner->verticalScrollBar();
  _scrollTop = bar->value();
  connect(bar, &QScrollBar::valueChanged, [=] { _scrollTop = bar->value(); });
...
void InputField::updatePalette() {
  ...
  if (!cursor.hasSelection()) {
    ...
    format.merge(PrepareTagFormat(_st, format.property(kTagProperty).toString()));
...
void InputField::setExtendedContextMenu(rpl::producer<ExtendedContextMenu> value) {
  std::move(value) | rpl::start_with_next(
                         [=](auto pair) {
                           auto &[menu, e] = pair;
...
void InputField::setInstantReplacesEnabled(rpl::producer<bool> enabled) {
  std::move(enabled) | rpl::start_with_next([=](bool value) { _instantReplacesEnabled = value; }, lifetime());
}

void InputField::setMarkdownReplacesEnabled(rpl::producer<bool> enabled) {
  std::move(enabled) | rpl::start_with_next(
                           [=](bool value) {
...
void InputField::setTagMimeProcessor(std::unique_ptr<TagMimeProcessor> &&processor) {
  _tagMimeProcessor = std::move(processor);
}

void InputField::setAdditionalMargin(int margin) {
  _inner->setStyleSheet(QString::fromLatin1("QTextEdit { margin: %1px; }").arg(margin));
    ...
    for (auto i = position - fragmentPosition; i > 0; --i) {
      ...
      if (previous == '@' || previous == '#' || previous == '/') {
        if ((i == position - fragmentPosition ||
             (previous == '/' ? fragmentText[i].isLetterOrNumber() : fragmentText[i].isLetter()) || previous == '#') &&
            (i < 2 || !(fragmentText[i - 2].isLetterOrNumber() || fragmentText[i - 2] == '_'))) {
          ...
          if (till < fragmentEnd && fragmentText[till - fragmentPosition] == ' ') {
            ++till;
          }
          cursor.setPosition(till, QTextCursor::KeepAnchor);
          break;
        } else if ((i == position - fragmentPosition || fragmentText[i].isLetter()) && fragmentText[i - 1] == '@' &&
                   (i > 2) && (fragmentText[i - 2].isLetterOrNumber() || fragmentText[i - 2] == '_') &&
                   !mentionInCommand) {
      ...
      if (position - fragmentPosition - i > 127 || (!mentionInCommand && (position - fragmentPosition - i > 63)) ||
          (!fragmentText[i - 1].isLetterOrNumber() && fragmentText[i - 1] != '_')) {
        break;
      }
    }
    break;
...
bool InputField::heightAutoupdated() {
  if (_minHeight < 0 || _maxHeight < 0 || _inHeightCheck || _mode == Mode::SingleLine) {
    return false;
  }
  _inHeightCheck = true;
  ...
  const auto contentHeight = int(std::ceil(document()->size().height())) + _st.textMargins.top() +
                             _st.textMargins.bottom() + 2 * _additionalMargin;
  ...
  if (_st.placeholderScale > 0. && !_placeholderPath.isEmpty()) {
    ...
    if (placeholderHiddenDegree < 1.) {
      ...
      if (_st.placeholderAlign == style::al_topleft && _placeholderAfterSymbols > 0) {
        ...
        p.drawText(_st.textMargins.left() + _st.placeholderMargins.left() + skipWidth,
                   _st.textMargins.top() + _st.placeholderMargins.top() + _st.placeholderFont->ascent, _placeholder);
      } else {
...
QString InputField::getTextPart(int start, int end, TagList &outTagsList, bool &outTagsChanged,
  ...
  for (auto block = from; block != till;) {
    for (auto item = block.begin(); !item.atEnd(); ++item) {
      ...
      const auto fragmentEnd = full ? 0 : (fragmentPosition + fragment.length());
      const auto format = fragment.charFormat();
      if (!full) {
        if (fragmentPosition == end) {
          tagAccumulator.feed(format.property(kTagProperty).toString(), result.size());
          break;
...
void InputField::processFormatting(int insertPosition, int insertEnd) {
  ...
  const auto breakTagOnNotLetterTill =
      ProcessInsertedTags(_st, document, insertPosition, insertEnd, _insertedTags, insertedTagsProcessor);
  ...
  while (true) {
    ...
    for (auto block = fromBlock; block != tillBlock; block = block.next()) {
      for (auto fragmentIt = block.begin(); !fragmentIt.atEnd(); ++fragmentIt) {
        ...
        if (with.isValid()) {
          ...
          if (fragmentText != string) {
            ...
            action.intervalStart = fragmentPosition + (fragmentText.startsWith(string) ? string.size() : 0);
        ...
        for (; ch < textEnd; ++ch) {
          const auto removeNewline = (_mode != Mode::MultiLine) && IsNewline(*ch);
          if (removeNewline) {
      ...
      if (action.type != ActionType::Invalid) {
        break;
      } else if (_mode != Mode::MultiLine && block.next() != document->end()) {
        action.type = ActionType::RemoveNewline;
    ...
    if (action.type != ActionType::Invalid) {
      ...
      auto cursor = QTextCursor(document->docHandle(), action.intervalStart);
      ...
      if (action.type == ActionType::InsertEmoji) {
        ...
        if (insertEnd >= action.intervalEnd) {
          insertEnd -= action.intervalEnd - action.intervalStart - 1;
        }
      } else if (action.type == ActionType::RemoveTag) {
        RemoveDocumentTags(_st, document, action.intervalStart, action.intervalEnd);
      } else if (action.type == ActionType::TildeFont) {
        auto format = QTextCharFormat();
        format.setFont(action.isTilde ? tildeFixedFont : PrepareTagFormat(_st, action.tildeTag).font());
        cursor.mergeCharFormat(format);
...
void InputField::onDocumentContentsChange(int position, int charsRemoved, int charsAdded) {
  ...
  const auto insertPosition = (_realInsertPosition >= 0) ? _realInsertPosition : position;
  const auto insertLength = (_realInsertPosition >= 0) ? _realCharsAdded : charsAdded;
  ...
  if (toRemove > 0) {
    if (toRemove > insertLength) {
      if (insertLength) {
        ...
        cursor.setPosition((insertPosition + insertLength), QTextCursor::KeepAnchor);
        cursor.removeSelectedText();
      }
      ...
      cursor.setPosition(insertPosition + (insertLength - toRemove));
      cursor.setPosition(insertPosition + insertLength, QTextCursor::KeepAnchor);
      cursor.removeSelectedText();
    }
  }
...
void InputField::handleContentsChanged() {
  ...
  const auto currentText =
      getTextPart(0, -1, _lastTextWithTags.tags, tagsChanged, _markdownEnabled ? &_lastMarkdownTags : nullptr);
...
void InputField::highlightMarkdown() {
  ...
  for (const auto &tag : _lastMarkdownTags) {
    ...
    applyColor(tag.internalStart, tag.internalStart + tag.internalLength,
               (tag.closed ? QColor(0, 128, 0) : QColor(128, 0, 0)));
  }
  auto cursor = textCursor();
  cursor.movePosition(QTextCursor::End);
...
void InputField::startPlaceholderAnimation() {
  const auto textLength = [&] { return getTextWithTags().text.size() + _lastPreEditText.size(); };
  const auto placeholderShifted =
      _forcePlaceholderHidden || (_focused && _st.placeholderScale > 0.) || (textLength() > _placeholderAfterSymbols);
  if (_placeholderShifted != placeholderShifted) {
    _placeholderShifted = placeholderShifted;
    _a_placeholderShifted.start([=] { update(); }, _placeholderShifted ? 0. : 1., _placeholderShifted ? 1. : 0.,
                                _st.duration);
  }
}
...
QMimeData *InputField::createMimeDataFromSelectionInner() const {
  ...
  return TextUtilities::MimeDataFromText((end > start) ? getTextWithTagsPart(start, end) : TextWithTags()).release();
}
...
void InputField::setTextWithTags(const TextWithTags &textWithTags, HistoryAction historyAction) {
...
TextWithTags InputField::getTextWithAppliedMarkdown() const {
  ...
  const auto links = TextUtilities::ParseEntities(originalText, 0).entities;
  ...
  const auto addOriginalTagsUpTill = [&](int offset) {
    while (originalTag != originalTagsEnd && originalTag->offset + originalTag->length <= offset) {
  ...
  for (const auto &tag : _lastMarkdownTags) {
    ...
    if (originalTag != originalTagsEnd && originalTag->offset < tagAdjustedEnd) {
      continue;
    }
    while (link != linksEnd && link->offset() + link->length() <= tag.adjustedStart) {
      ++link;
    }
    if (link != linksEnd && link->offset() < tagAdjustedEnd &&
        (link->offset() + link->length() > tagAdjustedEnd || link->offset() < tag.adjustedStart)) {
      continue;
    }
    ...
    if (tag.tag == kTagPre) {
      ...
      if (IsNewline(originalText[entityStart]) &&
          (result.text.isEmpty() || IsNewline(result.text[result.text.size() - 1]))) {
        ++entityStart;
        --entityLength;
      }
      const auto entityEnd = entityStart + entityLength;
      if (IsNewline(originalText[entityEnd - 1]) &&
          (originalText.size() <= entityEnd + tagLength || IsNewline(originalText[entityEnd + tagLength]))) {
        --entityLength;
      }
    }
    ...
    if (entityLength > 0) {
      ...
      result.tags.push_back(TextWithTags::Tag{int(result.text.size()), entityLength, tag.tag});
      result.text.append(originalText.midRef(entityStart, entityLength));
    }
...
bool InputField::ShouldSubmit(SubmitSettings settings, Qt::KeyboardModifiers modifiers) {
  ...
  return (ctrl && shift) || (ctrl && settings != SubmitSettings::None && settings != SubmitSettings::Enter) ||
         (!ctrl && !shift && settings != SubmitSettings::None && settings != SubmitSettings::CtrlEnter);
}
...
void InputField::keyPressEventInner(QKeyEvent *e) {
  ...
  bool macmeta = Platform::IsMac() && e->modifiers().testFlag(Qt::ControlModifier) &&
                 !e->modifiers().testFlag(Qt::MetaModifier) && !e->modifiers().testFlag(Qt::AltModifier);
  bool ctrl = e->modifiers().testFlag(Qt::ControlModifier) || e->modifiers().testFlag(Qt::MetaModifier);
  bool enterSubmit = (_mode != Mode::MultiLine) || ShouldSubmit(_submitSettings, e->modifiers());
  ...
  if (macmeta && e->key() == Qt::Key_Backspace) {
  ...
  } else if (e->key() == Qt::Key_Backspace && e->modifiers() == 0 && revertFormatReplace()) {
    e->accept();
    ...
    e->accept();
  } else if (_customUpDown && (e->key() == Qt::Key_Up || e->key() == Qt::Key_Down || e->key() == Qt::Key_PageUp ||
                               e->key() == Qt::Key_PageDown)) {
    e->ignore();
#ifdef Q_OS_MAC
  } else if (e->key() == Qt::Key_E && e->modifiers().testFlag(Qt::ControlModifier)) {
    ...
    const auto end = cursor.selectionEnd();
    if (end > start) {
      QGuiApplication::clipboard()->setText(getTextWithTagsPart(start, end).text, QClipboard::FindBuffer);
    }
#endif  // Q_OS_MAC
  } else {
    ...
    if (cursor.position() == oldPosition) {
      ...
      if (e->key() == Qt::Key_PageUp || e->key() == Qt::Key_Up) {
        cursor.movePosition(QTextCursor::Start, e->modifiers().testFlag(Qt::ShiftModifier) ? QTextCursor::KeepAnchor
                                                                                           : QTextCursor::MoveAnchor);
        check = true;
      } else if (e->key() == Qt::Key_PageDown || e->key() == Qt::Key_Down) {
        cursor.movePosition(QTextCursor::End, e->modifiers().testFlag(Qt::ShiftModifier) ? QTextCursor::KeepAnchor
                                                                                         : QTextCursor::MoveAnchor);
        check = true;
...
bool InputField::handleMarkdownKey(QKeyEvent *e) {
  ...
  const auto matches = [&](const QKeySequence &sequence) {
    const auto searchKey = (e->modifiers() | e->key()) & ~(Qt::KeypadModifier | Qt::GroupSwitchModifier);
  ...
  if (e == QKeySequence::Bold) {
    ...
    const auto cursor = textCursor();
    editMarkdownLink({cursor.selectionStart(), cursor.selectionEnd()});
  } else {
    return false;
  }
  return true;
...
auto InputField::selectionEditLinkData(EditLinkSelection selection) const -> EditLinkData {
  Expects(_editLinkCallback != nullptr);

  const auto position =
      (selection.from == selection.till && selection.from > 0) ? (selection.from - 1) : selection.from;
  const auto link = [&] {
    return (position != selection.till) ? GetFullSimpleTextTag(getTextWithTagsPart(position, selection.till))
                                        : QString();
  }();
  const auto simple = EditLinkData{selection.from, selection.till, QString()};
  ...
  const auto stateStart = [&](const State &state) { return state.i.fragment().position(); };
...
auto InputField::editLinkSelection(QContextMenuEvent *e) const -> EditLinkSelection {
  const auto cursor = textCursor();
  if (!cursor.hasSelection() && e->reason() == QContextMenuEvent::Mouse) {
    const auto clickCursor = _inner->cursorForPosition(_inner->viewport()->mapFromGlobal(e->globalPos()));
    if (!clickCursor.isNull() && !clickCursor.hasSelection()) {
      return {clickCursor.position(), clickCursor.position()};
    }
  }
...
void InputField::editMarkdownLink(EditLinkSelection selection) {
  ...
  _editLinkCallback(selection, getTextWithTagsPart(data.from, data.till).text, data.link, EditLinkAction::Edit);
}
...
void InputField::processInstantReplaces(const QString &appended) {
  ...
  if (appended.size() != 1 || !_instantReplacesEnabled || !replaces.maxLength) {
    return;
  }
  ...
  const auto position = textCursor().position();
  for (const auto &tag : _lastMarkdownTags) {
    if (tag.internalStart < position && tag.internalStart + tag.internalLength >= position &&
        (tag.tag == kTagCode || tag.tag == kTagPre)) {
      return;
    }
  }
  const auto typed = getTextWithTagsPart(std::max(position - replaces.maxLength, 0), position - 1).text;
  auto node = &it->second;
  auto i = typed.size();
  do {
...
void InputField::applyInstantReplace(const QString &what, const QString &with) {
...
void InputField::commitInstantReplacement(int from, int till, const QString &with) {
  commitInstantReplacement(from, till, with, std::nullopt, false);
}

void InputField::commitInstantReplacement(int from, int till, const QString &with, std::optional<QString> checkOriginal,
                                          bool checkIfInMonospace) {
  const auto original = getTextWithTagsPart(from, till).text;
  if (checkOriginal && checkOriginal->compare(original, Qt::CaseInsensitive) != 0) {
    return;
  }
  ...
  auto format = [&]() -> QTextCharFormat {
    ...
    const auto use = Integration::Instance().defaultEmojiVariant(emoji);
    return PrepareEmojiFormat(use, _st.font);
  }();
  const auto replacement = format.isImageFormat() ? kObjectReplacement : with;
  format.setProperty(kInstantReplaceWhatId, original);
  format.setProperty(kInstantReplaceWithId, replacement);
  format.setProperty(kInstantReplaceRandomId, openssl::RandomValue<uint32>());
  ApplyTagFormat(format, cursor.charFormat());
  cursor.insertText(replacement, format);
}

bool InputField::commitMarkdownReplacement(int from, int till, const QString &tag, const QString &edge) {
  const auto end = [&] {
    auto cursor = QTextCursor(document()->docHandle(), 0);
    cursor.movePosition(QTextCursor::End);
  ...
  const auto extended = getTextWithTagsPart(from - extendLeft, till + extendRight).text;
  const auto outer = extended.midRef(extendLeft, extended.size() - extendLeft - extendRight);
  if ((outer.size() <= 2 * edge.size()) || (!edge.isEmpty() && !(outer.startsWith(edge) && outer.endsWith(edge)))) {
    return false;
  }
  ...
  const auto checkIfTwoNewlines = blocktag && (extendLeft > 0) && IsNewline(extended[0]);
  ...
  const auto newlineleft = blocktag && (extendLeft > 0) && !IsNewline(extended[0]) && !IsNewline(outer.at(innerLeft));
  const auto newlineright = blocktag && (!extendRight || !IsNewline(extended[extended.size() - 1])) &&
...
void InputField::commitMarkdownLinkEdit(EditLinkSelection selection, const QString &text, const QString &link) {
  if (text.isEmpty() || !IsValidMarkdownLink(link) || !_editLinkCallback) {
    return;
  }
  _insertedTags.clear();
  ...
  auto format = _defaultCharFormat;
  _insertedTagsAreFromMime = false;
  cursor.insertText((editData.from == editData.till) ? (text + QChar(' ')) : text, _defaultCharFormat);
  _insertedTags.clear();
...
void InputField::toggleSelectionMarkdown(const QString &tag) {
  ...
  if (tag.isEmpty() || GetFullSimpleTextTag(getTextWithTagsSelected()) == tag) {
  ...
  const auto commitTag = [&] {
    ...
    const auto leftForBlock = [&] {
      ...
      return text.isEmpty() || IsNewline(text[0]) || IsNewline(text[text.size() - 1]);
    }();
...
bool InputField::revertFormatReplace() {
  ...
  for (auto i = block.begin(); !i.atEnd(); ++i) {
    ...
    if (current.hasProperty(kInstantReplaceWithId)) {
      ...
      return true;
    } else if (_reverseMarkdownReplacement && current.hasProperty(kReplaceTagId)) {
      ...
      replaceCursor.insertText(tag + getTextWithTagsPart(from, till).text + tag, _defaultCharFormat);
      return true;
    }
    return false;
  }
...
void InputField::addMarkdownActions(not_null<QMenu *> menu, QContextMenuEvent *e) {
  if (!_markdownEnabled) {
    return;
  }
  auto &integration = Integration::Instance();

  const auto formatting = new QAction(integration.phraseFormattingTitle(), menu);
  ...
  const auto add = [&](const QString &base, QKeySequence sequence, bool disabled, auto callback) {
    const auto add = sequence.isEmpty() ? QString() : QChar('\t') + sequence.toString(QKeySequence::NativeText);
    const auto action = new QAction(base + add, submenu);
    connect(action, &QAction::triggered, this, callback);
    action->setDisabled(disabled);
    submenu->addAction(action);
  };
  const auto addtag = [&](const QString &base, QKeySequence sequence, const QString &tag) {
    const auto disabled = (fullTag == tag) || (fullTag == kTagPre && tag == kTagCode);
    add(base, sequence, (!hasText || fullTag == tag), [=] { toggleSelectionMarkdown(tag); });
  };
  const auto addlink = [&] {
    const auto selection = editLinkSelection(e);
    const auto data = selectionEditLinkData(selection);
    const auto base =
        data.link.isEmpty() ? integration.phraseFormattingLinkCreate() : integration.phraseFormattingLinkEdit();
    add(base, kEditLinkSequence, false, [=] { editMarkdownLink(selection); });
  };
  const auto addclear = [&] {
    const auto disabled = !hasText || !hasTags;
    add(integration.phraseFormattingClear(), kClearFormatSequence, disabled, [=] { clearSelectionMarkdown(); });
  };
...
void InputField::addMarkdownMenuAction(not_null<QMenu *> menu, not_null<QAction *> action) {
...
bool InputField::canInsertFromMimeDataInner(const QMimeData *source) const {
  if (source && _mimeDataHook && _mimeDataHook(source, MimeAction::Check)) {
    return true;
  }
...
void InputField::insertFromMimeDataInner(const QMimeData *source) {
  ...
  const auto text = [&] {
    ...
    _insertedTags = TextUtilities::DeserializeTags(source->data(tagsMime), result.size());
...
void InputField::refreshPlaceholder(const QString &text) {
  const auto availableWidth = width() - _st.textMargins.left() - _st.textMargins.right() -
                              _st.placeholderMargins.left() - _st.placeholderMargins.right() - 1;
...
void InputField::setPlaceholder(rpl::producer<QString> placeholder, int afterSymbols) {
  _placeholderFull = std::move(placeholder);
...
void InputField::setEditLinkCallback(
    Fn<bool(EditLinkSelection selection, QString text, QString link, EditLinkAction action)> callback) {
...
MaskedInputField::MaskedInputField(QWidget *parent, const style::InputField &st, rpl::producer<QString> placeholder,
                                   const QString &val)
    : Parent(val, parent), _st(st), _oldtext(val), _placeholderFull(std::move(placeholder)) {
  ...
  connect(this, SIGNAL(textEdited(const QString &)), this, SLOT(onTextEdited()));
  connect(this, &MaskedInputField::selectionChanged, [] { Integration::Instance().textActionsUpdated(); });
...
void MaskedInputField::refreshPlaceholder(const QString &text) {
  const auto availableWidth = width() - _textMargins.left() - _textMargins.right() - _st.placeholderMargins.left() -
...
PasswordInput::PasswordInput(QWidget *parent, const style::InputField &st, rpl::producer<QString> placeholder,
                             const QString &val)
    : MaskedInputField(parent, st, std::move(placeholder), val) {
  setEchoMode(QLineEdit::Password);
}

PortInput::PortInput(QWidget *parent, const style::InputField &st, rpl::producer<QString> placeholder,
                     const QString &val)
    : MaskedInputField(parent, st, std::move(placeholder), val) {
...
void PortInput::correctValue(const QString &was, int wasCursor, QString &now, int &nowCursor) {
  QString newText;
...
HexInput::HexInput(QWidget *parent, const style::InputField &st, rpl::producer<QString> placeholder, const QString &val)
    : MaskedInputField(parent, st, std::move(placeholder), val) {
...
void HexInput::correctValue(const QString &was, int wasCursor, QString &now, int &nowCursor) {
  QString newText;
  newText.reserve(now.size());
  auto newPos = nowCursor;
  for (auto i = 0, l = now.size(); i < l; ++i) {
