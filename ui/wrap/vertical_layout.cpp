...
QMargins VerticalLayout::getMargins() const {
  ...
  if (!_rows.empty()) {
    ...
    result.setTop(qMax(topMargin - top.margin.top(), 0));
    auto &bottom = _rows.back();
    auto bottomMargin = bottom.widget->getMargins().bottom();
    result.setBottom(qMax(bottomMargin - bottom.margin.bottom(), 0));
    for (auto &row : _rows) {
      auto margins = row.widget->getMargins();
      result.setLeft(qMax(margins.left() - row.margin.left(), result.left()));
      result.setRight(qMax(margins.right() - row.margin.right(), result.right()));
    }
  }
  return result;
}
...
int VerticalLayout::naturalWidth() const {
  ...
  for (auto &row : _rows) {
    ...
    accumulate_max(result, row.margin.left() + natural + row.margin.right());
  }
  return result;
}
...
int VerticalLayout::resizeGetHeight(int newWidth) {
  ...
  auto result = 0;
  for (auto &row : _rows) {
    updateChildGeometry(margins, row.widget, row.margin, newWidth, result + row.verticalShift);
    result += row.margin.top() + row.widget->heightNoMargins() + row.margin.bottom();
  }
  return result;
}

void VerticalLayout::visibleTopBottomUpdated(int visibleTop, int visibleBottom) {
  for (auto &row : _rows) {
    setChildVisibleTopBottom(row.widget, visibleTop, visibleBottom);
  }
}

void VerticalLayout::updateChildGeometry(const style::margins &margins, RpWidget *child, const style::margins &margin,
                                         int width, int top) const {
  auto availRowWidth = width - margin.left() - margin.right();
  child->resizeToNaturalWidth(availRowWidth);
  child->moveToLeft(margins.left() + margin.left(), margins.top() + margin.top() + top, width);
}

RpWidget *VerticalLayout::insertChild(int atPosition, object_ptr<RpWidget> child, const style::margins &margin) {
...
void VerticalLayout::childHeightUpdated(RpWidget *child) {
  auto it = ranges::find_if(_rows, [child](const Row &row) { return (row.widget == child); });
