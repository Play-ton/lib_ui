...
class FlatInput : public RpWidgetWrap<QLineEdit> {
 ...
 public:
  FlatInput(QWidget *parent, const style::FlatInput &st, rpl::producer<QString> placeholder = nullptr,
...
class InputField : public RpWidget {
 ...
 public:
  ...
  InputField(QWidget *parent, const style::InputField &st, rpl::producer<QString> placeholder,
             const QString &value = QString());
  InputField(QWidget *parent, const style::InputField &st, Mode mode, rpl::producer<QString> placeholder,
             const QString &value);
  InputField(QWidget *parent, const style::InputField &st, Mode mode = Mode::SingleLine,
             rpl::producer<QString> placeholder = nullptr, const TextWithTags &value = TextWithTags());
  ...
  void setTextWithTags(const TextWithTags &textWithTags, HistoryAction historyAction = HistoryAction::NewEntry);
  ...
  void commitMarkdownLinkEdit(EditLinkSelection selection, const QString &text, const QString &link);
  static bool IsValidMarkdownLink(const QString &link);

  const QString &getLastText() const {
    return _lastTextWithTags.text;
  }
  void setPlaceholder(rpl::producer<QString> placeholder, int afterSymbols = 0);
  ...
  static bool ShouldSubmit(SubmitSettings settings, Qt::KeyboardModifiers modifiers);
  void customUpDown(bool isCustom);
  ...
  using MimeDataHook = Fn<bool(not_null<const QMimeData *> data, MimeAction action)>;
 ...
 private:
  ...
  QString getTextPart(int start, int end, TagList &outTagsList, bool &outTagsChanged,
  ...
  void addMarkdownMenuAction(not_null<QMenu *> menu, not_null<QAction *> action);
  ...
  void commitInstantReplacement(int from, int till, const QString &with, std::optional<QString> checkOriginal,
                                bool checkIfInMonospace);
  bool commitMarkdownReplacement(int from, int till, const QString &tag, const QString &edge = QString());
  ...
  Fn<bool(EditLinkSelection selection, QString text, QString link, EditLinkAction action)> _editLinkCallback;
...
class MaskedInputField : public RpWidgetWrap<QLineEdit> {
 ...
 public:
  MaskedInputField(QWidget *parent, const style::InputField &st, rpl::producer<QString> placeholder = nullptr,
                   const QString &val = QString());
...
class PasswordInput : public MaskedInputField {
 public:
  PasswordInput(QWidget *parent, const style::InputField &st, rpl::producer<QString> placeholder = nullptr,
                const QString &val = QString());
};
