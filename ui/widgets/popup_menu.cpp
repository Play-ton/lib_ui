...
PopupMenu::PopupMenu(QWidget *parent, const style::PopupMenu &st)
    : RpWidget(parent), _st(st), _roundRect(ImageRoundRadius::Small, _st.menu.itemBg), _menu(this, _st.menu) {
  init();
}

PopupMenu::PopupMenu(QWidget *parent, QMenu *menu, const style::PopupMenu &st)
    : RpWidget(parent), _st(st), _roundRect(ImageRoundRadius::Small, _st.menu.itemBg), _menu(this, menu, _st.menu) {
  init();
...
void PopupMenu::init() {
  ...
  Integration::Instance().forcePopupMenuHideRequests() | rpl::start_with_next([=] { hideMenu(true); }, lifetime());
...
void PopupMenu::handleCompositingUpdate() {
  _padding =
      _useTransparency ? _st.shadow.extend : style::margins(st::lineWidth, st::lineWidth, st::lineWidth, st::lineWidth);
...
not_null<QAction *> PopupMenu::addAction(const QString &text, const QObject *receiver, const char *member,
                                         const style::icon *icon, const style::icon *iconOver) {
  return _menu->addAction(text, receiver, member, icon, iconOver);
}

not_null<QAction *> PopupMenu::addAction(const QString &text, Fn<void()> callback, const style::icon *icon,
...
void PopupMenu::popupSubmenu(SubmenuPointer submenu, int actionTop, TriggeredSource source) {
  ...
  if (submenu) {
    QPoint p(_inner.x() + (style::RightToLeft() ? _padding.right() : _inner.width() - _padding.left()),
             _inner.y() + actionTop);
...
void PopupMenu::showMenu(const QPoint &p, PopupMenu *parent, TriggeredSource source) {
  ...
  const auto forceLeft = _forcedOrigin && (*_forcedOrigin == Origin::TopLeft || *_forcedOrigin == Origin::BottomLeft);
  const auto forceTop = _forcedOrigin && (*_forcedOrigin == Origin::TopLeft || *_forcedOrigin == Origin::TopRight);
  const auto forceRight =
      _forcedOrigin && (*_forcedOrigin == Origin::TopRight || *_forcedOrigin == Origin::BottomRight);
  const auto forceBottom =
      _forcedOrigin && (*_forcedOrigin == Origin::BottomLeft || *_forcedOrigin == Origin::BottomRight);
...
PopupMenu::~PopupMenu() {
  ...
  if (const auto parent = parentWidget()) {
    if (QApplication::focusWidget() != nullptr && Ui::InFocusChain(parent->window())) {
