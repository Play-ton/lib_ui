...
CrossFadeAnimation::CrossFadeAnimation(style::color bg, Data &&was, Data &&now) : _bg(bg) {
  ...
  auto preparePart = [](const Data &data, int index, const Data &other) {
    ...
    auto snapshotRect = data.full.rect().intersected(
        QRect(left * pixelRatio, top * pixelRatio, lineWidth * pixelRatio, data.font->height * pixelRatio));
...
void CrossFadeAnimation::paintLine(QPainter &p, const Line &line, float64 positionReady, float64 alphaWas,
                                   float64 alphaNow) {
  ...
  if (!snapshotNow.isNull()) {
    ...
    if (widthDelta < 0) {
      p.fillRect(left + (snapshotNow.width() / pixelRatio), topWas, -widthDelta, snapshotWas.height() / pixelRatio,
                 _bg);
    }
  }
}
...
FlatLabel::FlatLabel(QWidget *parent, const style::FlatLabel &st)
    : RpWidget(parent), _text(st.minWidth ? st.minWidth : QFIXED_MAX), _st(st) {
  init();
}

FlatLabel::FlatLabel(QWidget *parent, const QString &text, const style::FlatLabel &st)
    : RpWidget(parent), _text(st.minWidth ? st.minWidth : QFIXED_MAX), _st(st) {
  setText(text);
  init();
}

FlatLabel::FlatLabel(QWidget *parent, rpl::producer<QString> &&text, const style::FlatLabel &st)
    : RpWidget(parent), _text(st.minWidth ? st.minWidth : QFIXED_MAX), _st(st) {
  textUpdated();
  std::move(text) | rpl::start_with_next([this](const QString &value) { setText(value); }, lifetime());
  init();
}

FlatLabel::FlatLabel(QWidget *parent, rpl::producer<TextWithEntities> &&text, const style::FlatLabel &st)
    : RpWidget(parent), _text(st.minWidth ? st.minWidth : QFIXED_MAX), _st(st) {
  textUpdated();
  std::move(text) | rpl::start_with_next([this](const TextWithEntities &value) { setMarkedText(value); }, lifetime());
  init();
}
...
int FlatLabel::countTextWidth() const {
  const auto available = _allowedWidth ? _allowedWidth : (_st.minWidth ? _st.minWidth : _text.maxWidth());
  if (_allowedWidth > 0 && _allowedWidth < _text.maxWidth() && _tryMakeSimilarLines) {
...
void FlatLabel::setLinksTrusted() {
  static const auto TrustedLinksFilter = [](const ClickHandlerPtr &link, Qt::MouseButton button) {
...
Text::StateResult FlatLabel::dragActionFinish(const QPoint &p, Qt::MouseButton button) {
  ...
  if (!_selection.empty()) {
    TextUtilities::SetClipboardText(_text.toTextForMimeData(_selection), QClipboard::Selection);
  }
...
void FlatLabel::showContextMenu(QContextMenuEvent *e, ContextMenuReason reason) {
  ...
  const auto uponSelection =
      _selectable && ((reason == ContextMenuReason::FromTouch && hasSelection) ||
                      (state.uponSymbol && (state.symbol >= _selection.from) && (state.symbol < _selection.to)));
  const auto fullSelection = _selectable && _text.isFullSelection(_selection);
  ...
  if (const auto link = ClickHandler::getActive()) {
    ...
    if (!actionText.isEmpty()) {
      _contextMenu->addAction(actionText,
...
CrossFadeAnimation::Data FlatLabel::crossFadeData(style::color bg, QPoint basePosition) {
...
std::unique_ptr<CrossFadeAnimation> FlatLabel::CrossFade(not_null<FlatLabel *> from, not_null<FlatLabel *> to,
                                                         style::color bg, QPoint fromPosition, QPoint toPosition) {
  return std::make_unique<CrossFadeAnimation>(bg, from->crossFadeData(bg, fromPosition),
...
void FlatLabel::paintEvent(QPaintEvent *e) {
  ...
  const auto textWidth = _textWidth ? _textWidth : (width() - _st.margin.left() - _st.margin.right());
  const auto textLeft = _textWidth ? ((_st.align & Qt::AlignLeft)      ? _st.margin.left()
                                      : (_st.align & Qt::AlignHCenter) ? ((width() - _textWidth) / 2)
  ...
  if (renderElided) {
    auto lineHeight = qMax(_st.style.lineHeight, _st.style.font->height);
    auto lines = _st.maxHeight ? qMax(_st.maxHeight / lineHeight, 1) : ((height() / lineHeight) + 2);
    _text.drawElided(p, textLeft, _st.margin.top(), textWidth, lines, _st.align, e->rect().y(), e->rect().bottom(), 0,
                     _breakEverywhere, selection);
  } else {
