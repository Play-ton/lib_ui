...
VerticalLayoutReorder::VerticalLayoutReorder(not_null<VerticalLayout *> layout, not_null<ScrollArea *> scroll)
    : _layout(layout), _scroll(scroll), _scrollAnimation([=] { updateScrollCallback(); }) {
}
...
void VerticalLayoutReorder::start() {
  ...
  for (auto i = 0; i != count; ++i) {
    ...
    const auto margins = _layout->marginsAt(i);
    widget->events()  //
        | rpl::start_with_next_done(
              [=](not_null<QEvent *> e) {
                switch (e->type()) {
                  case QEvent::MouseMove:
                    mouseMove(widget, dynamic_cast<QMouseEvent *>(e.get())->globalPos());
                    break;
                  case QEvent::MouseButtonPress:
                    mousePress(widget, dynamic_cast<QMouseEvent *>(e.get())->button(),
                               dynamic_cast<QMouseEvent *>(e.get())->globalPos());
                    break;
                  case QEvent::MouseButtonRelease:
                    mouseRelease(dynamic_cast<QMouseEvent *>(e.get())->button());
                    break;
                }
              },
              [=] { cancel(); }, _lifetime);
    _entries.push_back({widget, margins});
  }
}

void VerticalLayoutReorder::mouseMove(not_null<RpWidget *> widget, QPoint position) {
...
void VerticalLayoutReorder::updateOrder(int index, QPoint position) {
  ...
  const auto currentHeight = current.widget->height() + current.margins.top() + current.margins.bottom();
...
void VerticalLayoutReorder::mousePress(not_null<RpWidget *> widget, Qt::MouseButton button, QPoint position) {
...
void VerticalLayoutReorder::finishCurrent() {
  ...
  if (_currentDesiredIndex > 0 && _currentDesiredIndex >= _layout->count()) {
    _currentDesiredIndex = _layout->count() - 1;
  }

  base::reorder(_entries, index, _currentDesiredIndex);
...
void VerticalLayoutReorder::moveToShift(int index, int shift) {
  ...
  const auto widget = entry.widget;
  entry.shiftAnimation.start([=] { updateShift(widget, index); }, entry.finalShift, shift - entry.deltaShift,
                             st::slideWrapDuration);
  entry.finalShift = shift - entry.deltaShift;
}

void VerticalLayoutReorder::updateShift(not_null<RpWidget *> widget, int indexHint) {
  Expects(indexHint >= 0 && indexHint < _entries.size());

  const auto index = (_entries[indexHint].widget == widget) ? indexHint : indexOf(widget);
  auto &entry = _entries[index];
  entry.shift = std::round(entry.shiftAnimation.value(entry.finalShift)) + entry.deltaShift;
...
int VerticalLayoutReorder::deltaFromEdge() {
  ...
  const auto localBottom = localTop + _currentWidget->height() - _scroll->height();
